...
def browse_files():
    global directoryName, selectedFile, directory
...
#For each section in Section_Files dataframe, calculate section GPA and add to GPA total
  for i in range(0, (Section_Files.shape[0])):
    Current_Section = pd.read_csv(directoryName + Section_Files.iloc[i, 0], header = 1, names=['Name', 'Student_ID', 'Letter_Grade']);
    gpa_section = 0.00;
...
def main_program():
  ...
  for i in range(0, (Group_Files.shape[0])):
    ...
    for i in range(0, (Section_Files.shape[0])):
      ...
      #6) Define df 'Current_Section' which constructs a dataframe for the current section 
      Current_Section = pd.read_csv(directoryName + curr_section, header = 1, names=['Name', 'Student_ID', 'Letter_Grade']);
  ...
  #9) Define contents of Group Output dataframe
    ...
    global table, runTableDisplayed
...
def updateTableValues():
   ...
   if(section_combobox.current() != -1 & runTableDisplayed == True):
      # comboboxSelection = section_combobox.get()
      # selectedGroup = removeExtension(comboboxSelection)
...
section_label = tk.Label(frame, text="Select a section:")
section_label.grid(row=2, column=0, padx=5)
